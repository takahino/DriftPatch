...
ツールバーの **一括適用** をクリックすると、GUI を離れずに `driftpatch-batch` と同じ操作を実行できる。

1. **work ディレクトリ**・**パッチディレクトリ**・**レポートディレクトリ**を確認・編集する（設定から初期値が入る）。
2. **ドライラン** にチェックが入ったままだとファイルを変更せず適用可否のみ確認できる。チェックを外すと実際に適用される（無効時は警告が表示される）。
3. **ドライラン実行** / **適用実行** をクリックすると実行され、結果（サマリ件数とパッチごとのステータス）と生成された Excel/HTML レポートのパスが表示される。
4. **競合チェック** をクリックすると、設定したパッチディレクトリに対して `driftpatch-batch check` と同じ競合チェックを実行する。

実際に適用（非ドライラン）した場合、パッチ一覧が再読み込みされ、ファイルを開いている場合はディスクから再読み込みしてエディタに変更を反映する。
...
中央列にフォーカスがある状態で **Ctrl+F**（macOS では ⌘F）を押すと検索バーが開く。編集中テキストを検索でき、マッチ箇所はハイライトされ（カレントマッチは橙、それ以外は黄）、左右列は中央列のスクロール位置に追従するため同期してスクロールする。

| キー | 動作 |
|------|------|
| `Ctrl+F` | 検索を開く |
| `Enter` / `F3` | 次のマッチへジャンプ |
| `Shift+Enter` / `Shift+F3` | 前のマッチへジャンプ |
| `Esc` | 検索を閉じる |

「Aa」チェックボックスで大文字小文字の区別を切り替える（大文字小文字を区別しない検索は ASCII のみ対応）。

### ファイルを開く: ドラッグ&ドロップと最近使ったファイル

エクスプローラー/Finder からファイルを DriftPatch のウィンドウにドラッグすると開ける（ドラッグ中は「ドロップして開く」というオーバーレイが表示される）。ツールバーの **最近使ったファイル** メニューには最近開いたファイルが最大10件（新しい順）表示され、クリックすると再度開ける。**履歴をクリア** で一覧を空にできる。履歴中のファイルがディスク上に存在しない場合は、開こうとすると履歴から削除されエラーが表示される。
